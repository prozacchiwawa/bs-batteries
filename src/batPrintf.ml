...
let pad_string pad_char p neg s i len =
  ...
  if p <= len then Bytes.sub s i len else
    let res = Bytes.make p pad_char in
    if neg
    then Bytes.blit s i res 0 len
    else Bytes.blit s i res (p - len) len;
    res

(* Format a string given a %s format, e.g. %40s or %-20s.
   To do: ignore other flags (#, +, etc)? *)
let format_string sfmt s_ =
  let s = Bytes.of_string s_ in
  let (p, neg) = parse_string_conversion sfmt in
  Bytes.to_string (pad_string ' ' p neg s 0 (Bytes.length s)) ;;
...
let extract_format_int conv fmt start stop widths =
  let sfmt = Bytes.of_string (extract_format fmt start stop widths) in
  match conv with
  | 'n' | 'N' ->
    Bytes.set sfmt (Bytes.length sfmt - 1) 'u';
    Bytes.to_string sfmt
  | _ -> Bytes.to_string sfmt;;
